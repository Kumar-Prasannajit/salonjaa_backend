...
- Cancelling (`POST /bookings/:id/cancel`) a booking whose advance was paid forfeits it — no refund-request is possible on it (`POST /payments/refund-request` will `409`). **Updated by Module 20**: the forfeited amount is credited directly to the customer's wallet (`GET /wallet`) rather than the coupon this section originally documented — a notification event `ADVANCE_PAYMENT_FORFEITED_TO_WALLET` fires (carrying the amount), replacing the old `ADVANCE_PAYMENT_FORFEITED_COUPON_ISSUED`/coupon-code flow. See the wallet section below.
...
## General customer wallet — 🟢 Live (Module 20, `docs/NEXT_SESSION_PLAN.md` item 4)

Purpose: a stored-balance wallet, funded only by admin-approved refunds and Module 16 strikes-policy advance-forfeitures (both automatic — no top-up/add-funds endpoint exists). Spendable in full as a new `paymentMethod` option on booking creation. No withdrawal, no expiry.

- `GET /wallet` — customer-only. Success: `{ balance: number }`. A customer with no wallet activity yet gets `{ balance: 0 }`, not a 404.
- `GET /wallet/transactions` — customer-only. Success: array of `{ id, type: "CREDIT"|"DEBIT", amount, balanceAfter, reason: "BOOKING_PAYMENT"|"BOOKING_REFUND"|"REFUND_APPROVED"|"ADVANCE_FORFEITURE", referenceId, description, createdAt }`, newest first.
- `POST /bookings`'s `paymentMethod` now accepts `"WALLET"` alongside the existing `"ONLINE"`/`"PAY_AT_SALON"` — debits the full booking total immediately at creation time. `422` (no booking created) if the balance is insufficient — check `GET /wallet` first if you want to disable the option client-side before the customer tries it.
